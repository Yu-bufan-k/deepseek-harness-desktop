...
export class HarnessThemeStore {
  ...
  getPreference(): ThemePreference {
    try {
      ...
      for (const line of lines) {
        ...
        const match = line.match(/^\s+preference:\s*(light|dark|system)\s*(?:#.*)?$/);
        if (match && validPreferences.has(match[1] as ThemePreference)) return match[1] as ThemePreference;
      }
    } catch (error) {
  ...
  async setPreference(preference: ThemePreference): Promise<ThemePreference> {
    if (!validPreferences.has(preference)) throw new Error("Invalid theme preference");
    let content = "";
    try { content = await readFile(this.filePath, "utf8"); } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
    const lines = content.replace(/\r\n/g, "\n").split("\n");
    const themeIndex = lines.findIndex((line) => /^ui-theme:\s*(?:#.*)?$/.test(line));
    if (themeIndex === -1) {
      ...
      const preferenceIndex = lines.findIndex((line, index) => index > themeIndex && index < end && /^\s+preference:/.test(line));
      if (preferenceIndex === -1) lines.splice(themeIndex + 1, 0, `  preference: ${preference}`);
      else lines[preferenceIndex] = `${lines[preferenceIndex]!.match(/^\s*/)?.[0] ?? "  "}preference: ${preference}`;
    }
