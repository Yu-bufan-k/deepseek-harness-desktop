...
export class DeepSeekBalanceService {
  private snapshot: DeepSeekBalanceSnapshot = {
    configured: false,
    isAvailable: null,
    balances: [],
    checkedAt: null,
  ...
  async get(force = false): Promise<DeepSeekBalanceSnapshot> {
    ...
    const checked = this.snapshot.checkedAt
      ? Date.parse(this.snapshot.checkedAt)
      : 0;
    if (!force && checked && this.now() - checked < CACHE_MS)
      return structuredClone(this.snapshot);
    try {
      ...
      if (Buffer.byteLength(text) > MAX_RESPONSE_BYTES)
        throw new Error("DeepSeek 余额响应过大");
      ...
      const balances = value.balance_infos.map((entry) => {
        ...
        if (
          ...
          ![
            ...
            (amount) => typeof amount === "string" && DECIMAL.test(amount),
          )
        )
          throw new Error("DeepSeek 余额响应格式无效");
        const currency = item.currency as "CNY" | "USD";
        const totalBalance = item.total_balance as string,
      ...
      this.snapshot = {
        ...this.snapshot,
        configured: true,
        stale: Boolean(this.snapshot.checkedAt),
