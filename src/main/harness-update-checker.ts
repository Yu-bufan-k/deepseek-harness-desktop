...
export function isNewerHarnessVersion(
  ...
  for (
    let index = 0;
    index < Math.max(left.core.length, right.core.length);
    index += 1
  ) {
...
export async function checkHarnessUpdate(
  ...
  try {
    const response = await fetch(LATEST_DSH_URL, {
    ...
    if (
      ...
      throw new Error("npm registry returned an invalid Harness version");
    return {
      phase: "ready",
      currentVersion,
      latestVersion: value.version,
      updateAvailable: isNewerHarnessVersion(value.version, currentVersion),
      checkedAt,
      errorSummary: null,
    };
  } catch (error) {
