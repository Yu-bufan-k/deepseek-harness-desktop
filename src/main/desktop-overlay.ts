...
export async function writeDesktopOverlay(directory: string, pluginPath: string, billingPluginPath?: string): Promise<string> {
  await mkdir(directory, { recursive: true });
  const overlayPath = path.join(directory, "desktop.cordis.patch.yml");
  const pluginUrl = pathToFileURL(pluginPath).href;
  const billingPluginUrl = billingPluginPath ? pathToFileURL(billingPluginPath).href : null;
  const contents = [
    ...
    ...billingPluginUrl ? ["    - id: desktop-billing", `      name: ${yamlString(billingPluginUrl)}`] : [],
    ""
  ].join("\n");
