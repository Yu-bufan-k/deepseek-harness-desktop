...
describe("ChangeSetService", () => {
  it("captures a task baseline and safely reverts a modified file", async () => {
    ...
    await writeFile(path.join(root, "alpha.ts"), "export const alpha = 2;\nexport const beta = 3;\n", "utf8");
    const closed = await service.close(batch.id);
    expect(closed.files).toHaveLength(1);
    expect(closed.files[0]).toMatchObject({ path: "alpha.ts", kind: "modified", additions: 2, deletions: 1 });
    ...
    expect(await readFile(path.join(root, "alpha.ts"), "utf8")).toBe("export const alpha = 1;\n");
  });
  ...
  it("does not overwrite a file changed after review", async () => {
    ...
    await writeFile(path.join(root, "alpha.ts"), "export const alpha = 99;\n", "utf8");
    await expect(service.revertFile(batch.id, "alpha.ts")).rejects.toThrow("发生变化");
  ...
  it("removes files that were added by the task", async () => {
    ...
    const closed = await service.close(batch.id);
    expect(closed.files.find((file) => file.path === "new.ts")?.kind).toBe("added");
    await service.revertFile(batch.id, "new.ts");
    await expect(readFile(path.join(root, "new.ts"), "utf8")).rejects.toMatchObject({ code: "ENOENT" });
  });
  ...
  it("reverts one hunk without discarding a separate change", async () => {
    ...
    const changed = original.replace("line 2", "line two").replace("line 22", "line twenty-two");
    await writeFile(path.join(root, "alpha.ts"), changed, "utf8");
    const closed = await service.close(batch.id);
    expect(closed.files[0]?.hunks).toHaveLength(2);
    await service.revertHunk(batch.id, "alpha.ts", closed.files[0]!.hunks[0]!.id);
