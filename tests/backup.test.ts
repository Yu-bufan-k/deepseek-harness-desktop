...
describe("backup", () => {
  it("copies supported user data and records metadata", async () => {
    ...
    const destination = await createBackup(home, path.join(root, "backups"), {
      version: "1.0.0",
    });
    expect(
      await readFile(
        path.join(destination, "profiles", "profile.json"),
        "utf8",
      ),
    ).toBe("{}\n");
    expect(
      JSON.parse(await readFile(path.join(destination, "backup.json"), "utf8"))
        .version,
    ).toBe("1.0.0");
  });
});
