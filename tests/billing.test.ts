...
const amount = (line: ReturnType<typeof calculateBillingCost>) =>
  Number(BigInt(line.amountNanos!)) / 1e9;
...
describe("billing engine", () => {
  ...
  it("resolves named timezones with numeric parts", () => {
    expect(
      billingMinuteAt(Date.parse("2026-08-17T01:30:00Z"), "Asia/Shanghai"),
    ).toBe(570);
  ...
  it("uses all four off-peak and peak rates", () => {
    expect(
      amount(
        calculateBillingCost(settings, sample("2026-08-17T08:00:00+08:00")),
      ),
    ).toBe(6.25);
    expect(
      amount(
        calculateBillingCost(settings, sample("2026-08-17T09:30:00+08:00")),
      ),
    ).toBe(12.5);
  });
  ...
  it("does not price third-party routes or requests after effectiveTo", () => {
    expect(
      selectBillingRule(settings, {
        ...sample("2026-08-17T09:30:00+08:00"),
        provider: "fireworks",
      }),
    ).toBeNull();
    expect(
      selectBillingRule(settings, sample("2027-01-01T00:00:00+08:00")),
    ).toBeNull();
  });
  ...
  it("lets an active custom rule override an official version", () => {
    const custom = {
      ...
      effectiveTo: "2026-08-20T00:00:00+08:00",
      rates: rates(1, 0, 2, 1),
      peakRates: undefined,
    ...
    expect(
      selectBillingRule(
        { ...settings, customRules: [custom] },
        sample("2026-08-18T10:00:00+08:00"),
      )?.id,
    ).toBe("custom");
    expect(
      selectBillingRule(
        { ...settings, customRules: [custom] },
        sample("2026-08-21T10:00:00+08:00"),
      )?.id,
    ).toBe("official");
  });
  ...
  it("binds a custom route to official pricing and restores without rewriting history", () => {
    const bound = {
      ...settings,
      providerBindings: [
        { provider: "my-deepseek", catalogProvider: "deepseek-official" },
      ],
    };
    const routed = {
      ...sample("2026-08-18T10:00:00+08:00"),
    ...
    const custom = {
      ...
      provider: "my-deepseek",
      mode: "custom" as const,
      effectiveFrom: "2026-08-18T00:00:00+08:00",
      effectiveTo: undefined,
      peakRates: undefined,
      peakScheduleId: undefined,
    };
    const restored = restoreOfficialBilling(
      { ...bound, customRules: [custom] },
      "my-deepseek",
      custom.model,
      Date.parse("2026-08-19T00:00:00+08:00"),
    );
    expect(restored.customRules[0]?.effectiveTo).toBe(
      "2026-08-18T16:00:00.000Z",
    );
    expect(
      selectBillingRule(restored, {
        ...routed,
        time: Date.parse("2026-08-18T12:00:00+08:00"),
      })?.id,
    ).toBe("custom-route");
    expect(
      selectBillingRule(restored, {
        ...routed,
        time: Date.parse("2026-08-19T00:01:00+08:00"),
      })?.mode,
    ).toBe("official");
  });
  ...
  it("derives rule status from the same selector used for pricing", () => {
    ...
    expect(
      billingRuleStatus(
        settings,
        settings.catalog.rules[0]!,
        Date.parse("2026-08-16T00:00:00+08:00"),
      ),
    ).toBe("future");
    expect(
      billingRuleStatus(
        overridden,
        settings.catalog.rules[0]!,
        Date.parse("2026-08-18T00:00:00+08:00"),
      ),
    ).toBe("overridden");
    expect(
      billingRuleStatus(
    ...
    expect(
      billingRuleStatus(
        settings,
        settings.catalog.rules[0]!,
        Date.parse("2027-01-01T00:00:00+08:00"),
      ),
    ).toBe("expired");
    const historical = {
      ...settings.catalog.rules[0]!,
      ...
      billingRuleStatus(
        {
          ...
          catalog: {
            ...settings.catalog,
            rules: [historical, settings.catalog.rules[0]!],
          },
        },
        historical,
        Date.parse("2026-08-18T00:00:00+08:00"),
  ...
  it("formats amounts above Number.MAX_SAFE_INTEGER without floating-point conversion", () => {
    expect(formatBillingMoney("CNY", "9007199254740993")).toBe(
      "¥9,007,199.254741",
    );
  ...
  it("adds many tiny costs as integer nanounits", () => {
        ...
        sessions: [
          {
            ...
            title: "tiny",
            revision: "1",
            samples: Array.from({ length: 100_000 }, () => tiny),
          },
        ],
      },
    ...
    expect(report.totals).toEqual([
      { currency: "CNY", nanos: "5000000", display: "¥0.0050" },
    ]);
  ...
  it("reuses unchanged session summaries and invalidates only changed sessions", () => {
    ...
    summarizer.summarize(
      settings,
      { ...index, collectedAt: "2026-08-18T00:01:00Z" },
        ...
        sessions: [
          {
            ...first,
            revision: "2",
            samples: [...first.samples, sample("2026-08-18T08:01:00+08:00")],
          },
          second,
        ],
      },
